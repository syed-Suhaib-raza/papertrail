...
export default async function DecisionsIndexPage() {
  // 1) fetch assignments with status = 'submitted' and join paper + reviewer info
  ...
  // if no submitted assignments, show friendly empty state
  ...
  // 2) group assignments by paper_id and collect unique paper IDs
  ...
  // 3) fetch COI declarations for all these papers in one query
  ...
  // 4) Build a simple list of paper entries to render
  const papersToRender = paperIdList.map((pid) => {
    const assignmentsForPaper = byPaper.get(pid) ?? [];
    // take the joined paper object from the first assignment
    const rawPaperJoin = assignmentsForPaper[0]?.papers;
    // supabase returns nested joins usually as array; unwrap if necessary
